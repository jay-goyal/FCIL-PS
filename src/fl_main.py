...
from PneumoniaMNIST import PMnist
...
if args.dataset == "cifar100":
    ...
    test_dataset = iCIFAR100(
        "dataset", test_transform=test_transform, train=False, download=True
    )
...
log_str = "method_{}, task_size_{}, learning_rate_{}".format(
    args.method, args.task_size, args.learning_rate
)
...
for ep_g in range(args.epochs_global):
    ...
    if task_id != old_task_id and old_task_id != -1:
        ...
        old_client_1 = random.sample(
            [i for i in range(overall_client)], int(overall_client * 0.9)
        )
    ...
    for c in clients_index:
        local_model, proto_grad = local_train(
            models, c, model_g, task_id, model_old, ep_g, old_client_0
        )
    ...
    participant_exemplar_storing(
        models, num_clients, model_g, old_client_0, task_id, clients_index
    )
    ...
    out_file.flush()
    print(
        "classification accuracy of global model at round %d: %.3f \n"
        % (ep_g, acc_global)
    )
